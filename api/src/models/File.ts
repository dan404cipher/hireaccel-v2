...
export interface FileDocument extends Omit<IFileDocument, '_id' | 'uploadedBy'>, Document {
  ...
  // Instance methods
  logAccess(accessedBy: mongoose.Types.ObjectId, action: 'view' | 'download' | 'share' | 'delete', ipAddress?: string, userAgent?: string): void;
 ...
 * Add comment to file
 */
fileSchema.methods['addComment'] = function(
  this: FileDocument,
  content: string,
  createdBy: mongoose.Types.ObjectId
) {
...
fileSchema.methods['hasPermission'] = function(
  ...
  const permissionKey = `can${permission.charAt(0).toUpperCase() + permission.slice(1)}` as keyof typeof this.permissions;
