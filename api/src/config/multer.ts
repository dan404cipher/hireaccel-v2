...
/**
 * Sanitize untrusted path segments (role/entity) to avoid traversal or control chars
 * Only allow a-z, 0-9, dash and underscore. Lowercase and trim. Fallback to 'general'
 */
const sanitizeSegment = (input: unknown, fallback = 'general'): string => {
    const str = String(input || '').toLowerCase()
    const cleaned = str.replace(/[^a-z0-9_-]/g, '').slice(0, 40)
...
const generateFilePath = (role: string, entity: string, originalName: string): string => {
    ...
    const safeRole = sanitizeSegment(role, 'general')
    const safeEntity = sanitizeSegment(entity, 'files')
    ...
    return path.join(safeRole, safeEntity, year.toString(), month, sanitizedFilename)
}
...
const storage = multer.diskStorage({
    destination: (req, _file, cb) => {
        try {
            ...
            const role = sanitizeSegment(req.body.role || req.user?.role, 'general')
            const entity = sanitizeSegment(req.body.entity, 'files')
 ...
 */
const defaultFileFilter: multer.Options['fileFilter'] = (_req, _file, cb) => cb(null, true)

const createMulterConfig = (maxSize?: number, filter: multer.Options['fileFilter'] = defaultFileFilter) => ({
    storage,
    fileFilter: filter,
    limits: {
...
export const uploadImage = multer({
    ...createMulterConfig(
        ...
        createFileFilter(ALLOWED_FILE_TYPES.image.mimeTypes, ALLOWED_FILE_TYPES.image.extensions),
    ),
})
...
const dynamicFileFilter: multer.Options['fileFilter'] = (req: any, file, cb) => {
    try {
        const typeKey = sanitizeSegment(req.body.fileType, 'document') as keyof typeof ALLOWED_FILE_TYPES
        const allowed = ALLOWED_FILE_TYPES[typeKey] ?? ALLOWED_FILE_TYPES.document

        // MIME and extension checks
        if (!allowed.mimeTypes.includes(file.mimetype)) {
            return cb(new Error(`Invalid file type. Allowed types: ${allowed.mimeTypes.join(', ')}`))
        }
        const extension = path.extname(file.originalname).toLowerCase()
        if (!allowed.extensions.includes(extension)) {
            return cb(new Error(`Invalid file extension. Allowed extensions: ${allowed.extensions.join(', ')}`))
        }
        cb(null, true)
    } catch (err) {
        cb(err as Error)
