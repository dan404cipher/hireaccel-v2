...
const startServer = async (): Promise<void> => {
    ...
    const devOrigins = env.CORS_ORIGIN.split(',').map((o) => o.trim())
    ...
    // Tune HTTP server timeouts to mitigate slowloris and improve keep-alive
    httpServer.keepAliveTimeout = 65_000 // 65s keep-alive
    httpServer.headersTimeout = 66_000 // headers timeout slightly higher than keepAliveTimeout
    // Node 18+: requestTimeout controls overall request lifetime
    // @ts-ignore - property available at runtime in Node >= 18
    httpServer.requestTimeout = 60_000 // 60s overall request timeout
