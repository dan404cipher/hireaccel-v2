 ...
 */
export const validateRefreshTokenInDatabase = async (
  token: string, 
  ...
  try {
    ...
    const isValid = userRefreshTokens.some(rt => 
      hashToken(rt.token) === tokenHash
    );
...
export const rotateTokens = async (
  ...
  try {
    ...
    const isValidInDb = await validateRefreshTokenInDatabase(
      refreshToken, 
      user.refreshTokens
    );
