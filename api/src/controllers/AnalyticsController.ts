...
export class AnalyticsController {
    ...
    static track = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        ...
        const userId =
            req.user?._id || (validatedData.userId ? new mongoose.Types.ObjectId(validatedData.userId) : undefined);
    ...
    static getSummary = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        ...
        const avgDurationBounced =
            sessionStats.filter((s) => s.isBounced).reduce((sum, s) => sum + (s.duration || 0), 0) / bouncedSessions ||
            0;
        const avgDurationRegistered =
            registeredSessions.reduce((sum, s) => sum + (s.duration || 0), 0) / registeredSessions.length || 0;
    ...
    static getFunnels = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        ...
        const funnelWithDropoff = funnelData.map((stage, index) => {
            const previousCount = index > 0 ? funnelData[index - 1]?.count || 0 : stage.count;
        ...
        const conversionRate = totalVisitors > 0 ? (dashboardReached / totalVisitors) * 100 : 0;
    ...
    static getSources = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
                    ...
                    conversionRate: {
                        $cond: [
                            { $gt: [{ $size: '$uniqueSessions' }, 0] },
                            {
                                $multiply: [
                                    {
                                        $divide: ['$conversions', { $size: '$uniqueSessions' }],
                                    },
                                    100,
                                ],
                            },
        ...
        const directSessions = await AnalyticsEvent.distinct('sessionId', {
            timestamp: { $gte: start, $lte: end },
            $or: [{ referrer: null }, { referrer: '' }, { 'utm.source': { $exists: false } }],
        });

        const referralSessions = await AnalyticsEvent.distinct('sessionId', {
            timestamp: { $gte: start, $lte: end },
            referrer: { $exists: true, $ne: null, $nin: [''] },
        });
