...
export class UserController {
    ...
    static createAgentAssignment = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        ...
        if (validatedData.hrIds.length > 0) {
            ...
            console.log(
                '✅ Found active HR users:',
                hrUsers.map((u) => ({ id: u._id, email: u.email, role: u.role, status: u.status })),
            );

            if (hrUsers.length !== validatedData.hrIds.length) {
                const missingIds = validatedData.hrIds.filter((id) => !hrUsers.some((hr) => hr._id.toString() === id));
                const missingUsers = allUsersWithIds.filter((u) => missingIds.includes(u._id.toString()));
