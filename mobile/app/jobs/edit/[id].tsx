...
export default function EditJobScreen() {
    ...
    if (job) {
      setFormData({
        ...
        applicationDeadline: job.applicationDeadline
          ...
          : '',
        interviewRounds: job.interviewProcess?.rounds ? String(job.interviewProcess.rounds) : '2',
  ...
  useEffect(() => {
    if (formData.companyId && companies.length > 0) {
      const selectedCompany = companies.find((c: any) => c._id === formData.companyId || c.id === formData.companyId);
  ...
  const updateJobMutation = useMutation({
    mutationFn: (data: any) => apiClient.updateJob(job._id || job.id, data),
    onSuccess: () => {
        ...
        <View style={styles.field}>
          ...
          >
            <Text style={formData.companyId ? styles.selectorText : styles.selectorPlaceholder}>
              {formData.companyId
                ? companies.find((c: any) => c._id === formData.companyId || c.id === formData.companyId)?.name
                : 'Select Company'}
            </Text>
              ...
              >
                {companies.map((company: any) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setFormData({ ...formData, companyId: company._id || company.id });
                      setCompanyDialogVisible(false);
                    }}
                  >
                    <RadioButton value={company._id || company.id} />
